...
# преднамерено усложненный рекурсивный вариант с мемоизацией из коробки
...
Самым быстрым алгоритмом оказался алгоритм d (рекурсия с мемоизацией "из коробки").
{Почему не алгоритм a!?}
Однако стек вызовов быстро переполняется, в отличие от алгоритма b (с обычным циклом).
Лучшим вариантом можно считать алгоритм b, не имеющий ограничений по размеру n и 
несильно уступающий в скорости выполнения.
Приложение: les_4_task_1.xls.
'''
